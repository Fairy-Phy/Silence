...
client.on('message', message => {
    ...
    const otdel = message.guild.roles.find(role => role.id === roleids[2]);

    const delot = message.member.roles.has(otdel.id);

    if (delot) {
        console.log(`${message.author.tag}は${rolenames[2]}です`)
        return;
    }
    ...
    if (message.isMentioned(client.user)) {
        ...
        message.channel.send(`はい！何でしょうか？`)
        .then(() => {
...
client.on('message', message => {
    ...
    if (cmd === `onetime-add`) {
        ...
        if (otasu || otart) {
            ...
            console.log(`${message.author.tag}は${rolenames[0]},${rolenames[1]},${rolenames[3]}ではありません`)
            return;
        }
    }
            ...
            if (!member.user.bot) {
                rows.forEach((row) => {
                    if (row.role == 0) {
                        ...
                        const otrole = member.guild.roles.find(role => role.id === roleids[2]);
                        ...
                        let timeout = new Promise((resolve, reject) => {
                            setTimeout(function () {
                                member.send(`ワンタイムの時間が過ぎたためサーバーからキックされました`).then(function () {
                                    member.kick(`時間切れ`)
                                    .then(() => {
